...
# Self-contained: all artifacts stay inside this reference folder.
...
async def run(proxy: str | None = None) -> None:
    ...
    try:
        ...
        try:
            ...
            sc = BotScorecard.from_api_logger(api_logger)
            score_path = os.path.join(LOGS_DIR, f"{LEVEL_NAME}_{ts}_scorecard.xlsx")
            sc.save_to_excel(score_path)
            sc.print_report()
            print(f"\n  TARGET TO BEAT: 5/100 (see logs/GOLDEN_score_5_of_100_20260909.xlsx)")
            print(f"  THIS RUN:       {sc.detection_score}/100 — {sc.overall_verdict}")
            print(f"  Scorecard saved -> {score_path}")
        except Exception as score_exc:
            print(f"  (scorecard skipped: {score_exc})")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Override default ADDRESS for this run",
    )
