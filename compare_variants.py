"""Compare bot-detection signals across the 3 evasion-level runs.

Reads the filtered Excel files produced by verizon_plans_variants.py and
...
LEVEL_FILES = {
    "Level 1 (Naked Bot)": os.path.join(LOGS_DIR, "level1_naked_bot_filtered.xlsx"),
    "Level 2 (Basic Evasion)": os.path.join(LOGS_DIR, "level2_basic_evasion_filtered.xlsx"),
    "Level 3 (Stealth Mode)": os.path.join(LOGS_DIR, "level3_stealth_filtered.xlsx"),
}
...
def analyze_level(calls: list[dict], signals: list[dict], cookies: list[dict]) -> dict:
    """Extract key metrics from one level's data."""
    metrics: dict = {}
    ...
    # PerimeterX calls
    px_calls = [c for c in calls if "perimeter" in str(c.get("Host", "")).lower() or "px" in str(c.get("Bot/Fraud Vendor Match", "")).lower()]
    px_403 = [c for c in px_calls if c.get("Status") == 403]
    metrics["perimeterx_calls"] = len(px_calls)
    metrics["perimeterx_403s"] = len(px_403)

    # _abck cookie analysis
    ...
    # Unique bot/fraud vendors detected
    vendors: set[str] = set()
    for c in calls:
    ...
    # Sensor script reload frequency (scripts with sensor-like URLs)
    sensor_scripts = [
        c for c in calls
...
def print_comparison(all_metrics: dict[str, dict]) -> None:
    ...
    print("\n" + "=" * 90)
    print("   ANTI-BOT EVASION COMPARISON — SIDE BY SIDE")
    print("=" * 90)

    levels = list(all_metrics.keys())

    metric_labels = [
        ("total_important_calls", "Total Important Calls (H+M)"),
    ...
    print(f"\n{'Unique Vendors Detected:':<{label_width}}")
    for level in levels:
        short = level.split("(")[1].rstrip(")")
        vendors = all_metrics[level].get("unique_vendors_detected", [])
        print(f"  {short}: {', '.join(vendors) if vendors else 'none'}")
    ...
    if l3.get("http_403_count", 999) <= l1.get("http_403_count", 0):
        ...
        print("  ✗ Stealth mode has MORE 403 blocks (surprising)")
...
def write_comparison_excel(all_metrics: dict[str, dict], output_path: str) -> None:
    ...
    # Color code: lower is better for detection signals
    ...
    # Sheet 2: Detection Feedback breakdown
    ...
    for row_idx, fb in enumerate(sorted(all_feedbacks), 2):
        ...
        for col_idx, level in enumerate(levels, 2):
            ...
            ws2.cell(row=row_idx, column=col_idx, value=count).alignment = Alignment(horizontal="center")

    for col_idx in range(1, len(levels) + 2):
        ws2.column_dimensions[get_column_letter(col_idx)].width = 40 if col_idx == 1 else 20

    # Sheet 3: Signal Types breakdown
    ...
    for row_idx, st in enumerate(sorted(all_signal_types), 2):
        ...
        for col_idx, level in enumerate(levels, 2):
            ...
            ws3.cell(row=row_idx, column=col_idx, value=count).alignment = Alignment(horizontal="center")
...
def main() -> None:
    ...
    if missing:
        ...
        print("Run `python verizon_plans_variants.py --level N` first for each level.")
        return
    ...
    output_path = os.path.join(LOGS_DIR, "comparison_summary.xlsx")
